# 1、通达信先选出合适的票，存放在当日板块中，程序读取通达信的该板块的数据，得到股票列表
# 2、计算每个股票的积分，标识为预先分析，并入库
# 3、得到50分以上的数据保存为板块名.txt,内容为名称，代码，积分，评估时间

import os
import datetime
import pandas as pd
from stockrating.stock_rating_intraday import evaluate_stock
...
def calculate_and_save_block_scores(block_name):
    ...
    save_tdx_block_data(f"{block_name}02", stock_codes)
    # 保存到文件
