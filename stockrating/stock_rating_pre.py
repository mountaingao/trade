...
# 修改代码：从配置文件中获取tdxdir的值, 创建 Reader 对象
...
def load_cache(code, force_clear=0):
    """
    从本地文件加载缓存数据
    :param code: 股票代码
    :param force_clear: 是否强制清除缓存，1为强制清除，0为不强制
    :return: 缓存数据或None
    """
    if force_clear == 1:
        print(f"强制清除缓存，将重新抓取数据")
        return None
...
# 前一日预选积分条件说明
# 1、近期成交额（15%），分成3个级别：5日内平均成交额5-15亿以上100分，3-5亿 60分，3亿以下0分，15亿以上60分
# 2、近期涨幅（10%），分成3个级别：20日内涨幅30%以上 100分，20%以内 50分 10%以内 0分
# 3、流通市值（10%），分成3个级别：50亿-300亿 100分，300-500亿 50分，50亿以下 0分
# 4、振幅（10）分成3个级别：50% 100分，30
# 机构参与度
# 历史评分
# 用户关注指数
