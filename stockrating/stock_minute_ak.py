...
def get_daily_high(data: pd.DataFrame, date: str) -> float:
    ...
    daily_data = data[data.index.date == pd.to_datetime(date).date()]
    if daily_data.empty:
...
def calculate_volume_percentage(data: pd.DataFrame, timestamp: str) -> pd.Series:
    ...
    daily_data = data[data.index.date == target_time.date()]
    if daily_data.empty:
    ...
    # 获取指定时间点之前的数据
    filtered_data = daily_data[daily_data.index <= target_time]
    
    # 计算每分钟成交量占比
    filtered_data['volume_percentage'] = filtered_data['volume'] / total_volume
...
def calculate_percentage(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data['percentage'] = int(data['volume']) / int(total_volume)
    return data
...
def main(stock_code: str, days: int = 10):
    ...
    # 示例：获取2025-02-25的最高价
    try:
        high_price = get_daily_high(combined_data, "2025-02-25")
    ...
    try:
        volume_percentage = calculate_volume_percentage(combined_data, "2025-02-25 13:53:00")
